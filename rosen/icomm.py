...
crc32_table = [0x0, 0x4c11db7, 0x9823b6e, 0xd4326d9, 0x130476dc, 0x17c56b6b,
...
def crc32(byte_arr):
    ...
    for i in range(0,len(byte_arr)-4,4):
        word = (byte_arr[i]<<24) + (byte_arr[i+1]<<16) + (byte_arr[i+2]<<8) + byte_arr[i+3]
        crc = (crc << 8) ^ crc32_table[0xff & ((crc>>24) ^ word)]
        crc = (crc << 8) ^ crc32_table[0xff & ((crc>>24) ^ (word>>8))]
        crc = (crc << 8) ^ crc32_table[0xff & ((crc>>24) ^ (word>>16))]
        crc = (crc << 8) ^ crc32_table[0xff & ((crc>>24) ^ (word>>24))]
        crc = crc & mask
    
    remaining = len(byte_arr) % 4
    if remaining == 0:
        remaining = 4
    word = 0
    for i in range(remaining):
        word = word + (byte_arr[(len(byte_arr) - remaining) + i] << (24-i*8))
    word = word & mask
    crc = (crc << 8) ^ crc32_table[0xff & ((crc>>24) ^ word)]
    crc = (crc << 8) ^ crc32_table[0xff & ((crc>>24) ^ (word>>8))]
    crc = (crc << 8) ^ crc32_table[0xff & ((crc>>24) ^ (word>>16))]
...
icomm_construct = Padded(
    ...
    Struct(
        ...
        "checksum" / Checksum(
            Bytes(4),
            lambda data: crc32(data).to_bytes(4, 'big'),
            this.body.data
        )
    )
)
...
class ICOMMScript(Script):
    ...
    def execute(self, device, command):
        ...
        icomm_packet = ICOMM('cmd', device, payload=axe_packet)
